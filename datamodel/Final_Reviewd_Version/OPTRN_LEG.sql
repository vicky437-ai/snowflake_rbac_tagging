/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.OPTRN_LEG_BASE
================================================================================
Source Table : D_RAW.SADB.OPTRN_LEG_BASE
Target Table : D_BRONZE.SADB.OPTRN_LEG
Stream       : D_RAW.SADB.OPTRN_LEG_BASE_HIST_STREAM
Procedure    : D_RAW.SADB.SP_PROCESS_OPTRN_LEG()
Task         : D_RAW.SADB.TASK_PROCESS_OPTRN_LEG
Primary Key  : OPTRN_LEG_ID (Single)
Total Columns: 13 source + 6 CDC metadata = 19
================================================================================
*/
...
-- =============================================================================
ALTER TABLE D_RAW.SADB.OPTRN_LEG_BASE
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.OPTRN_LEG_BASE_HIST_STREAM
ON TABLE D_RAW.SADB.OPTRN_LEG_BASE
...
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_OPTRN_LEG()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
        ...
        FROM D_RAW.SADB.OPTRN_LEG_BASE_HIST_STREAM
        WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        USING (
            ...
            FROM D_RAW.SADB.OPTRN_LEG_BASE src
        ...
        WHEN NOT MATCHED THEN INSERT (
            ...
            TURN_LEG_SQNC_NBR, TYES_TRAIN_ID, SNW_OPERATION_TYPE, SNW_LAST_REPLICATED,
            ...
            src.TURN_LEG_SQNC_NBR, src.TYES_TRAIN_ID, src.SNW_OPERATION_TYPE, src.SNW_LAST_REPLICATED,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_OPTRN_LEG AS
    SELECT 
        OPTRN_LEG_ID, OPTRN_ID, TRAIN_DRCTN_CD, OPTRN_LEG_NM, MTP_TITAN_NBR,
        RECORD_CREATE_TMS, RECORD_UPDATE_TMS, CREATE_USER_ID, UPDATE_USER_ID,
        TURN_LEG_SQNC_NBR, TYES_TRAIN_ID, SNW_OPERATION_TYPE, SNW_LAST_REPLICATED,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_OPTRN_LEG;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_OPTRN_LEG;
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_PROCESS_OPTRN_LEG
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process OPTRN_LEG_BASE CDC changes into data preservation table'
WHEN
    SYSTEM$STREAM_HAS_DATA('D_RAW.SADB.OPTRN_LEG_BASE_HIST_STREAM')
AS
...
-- SHOW STREAMS LIKE 'OPTRN_LEG%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_PROCESS_OPTRN_LEG%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_OPTRN_LEG();

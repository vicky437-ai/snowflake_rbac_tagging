/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE
================================================================================
Source Table : D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE
Target Table : D_BRONZE.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT
...
Task         : D_RAW.AWS.TASK_SP_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT (
...
COMMENT = 'Bronze data preservation layer for Events Locomotive Movement Event.
Source: AWS PostgreSQL (events_locomotive_mvmnt_event) replicated via CDC into D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE.
Data Source Tables: D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE (25 source columns + 2 SNW metadata).
...
Pipeline Objects: Stream D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE_HIST_STREAM | Procedure D_RAW.AWS.SP_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT() | Task D_RAW.AWS.TASK_SP_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT.
...
-- =============================================================================
ALTER TABLE D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE_HIST_STREAM
ON TABLE D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.AWS.SP_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
        ...
        FROM D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE_HIST_STREAM
        WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE_HIST_STREAM src
        ) AS src
        ...
        WHEN NOT MATCHED THEN INSERT (
            "event_id", "event", "event_timestamp_utc", "source_system", "direction",
            "mark_cd", "eqpun_nbr", "fsac", "latitude", "longitude",
            "mile_km_number", "subdivision_name", "station_name", "train_plan_leg_id", "train_name",
            "scac", "division", "region", "district", "state_province",
            "country", "subdivision_id", "record_update_timestamp_utc",
            ...
            src."event_id", src."event", src."event_timestamp_utc", src."source_system", src."direction",
            src."mark_cd", src."eqpun_nbr", src."fsac", src."latitude", src."longitude",
            src."mile_km_number", src."subdivision_name", src."station_name", src."train_plan_leg_id", src."train_name",
            src."scac", src."division", src."region", src."district", src."state_province",
            src."country", src."subdivision_id", src."record_update_timestamp_utc",
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'EVENTS_LOCOMOTIVE_MVMNT_EVENT', :v_batch_id, 'RECOVERY', :v_start_time, :v_end_time,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_EVENTS_LOCOMOTIVE_MVMNT_EVENT AS
    SELECT 
        "event_id", "event", "event_timestamp_utc", "source_system", "direction",
        "mark_cd", "eqpun_nbr", "fsac", "latitude", "longitude",
        "mile_km_number", "subdivision_name", "station_name", "train_plan_leg_id", "train_name",
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_EVENTS_LOCOMOTIVE_MVMNT_EVENT;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_EVENTS_LOCOMOTIVE_MVMNT_EVENT;
        v_end_time := CURRENT_TIMESTAMP();
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'EVENTS_LOCOMOTIVE_MVMNT_EVENT', :v_batch_id, 'NO_DATA', :v_start_time, :v_end_time,
    ...
    FROM _CDC_STAGING_EVENTS_LOCOMOTIVE_MVMNT_EVENT;
    ...
    INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
        ...
        'EVENTS_LOCOMOTIVE_MVMNT_EVENT', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time,
...
EXCEPTION
    WHEN OTHER THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'EVENTS_LOCOMOTIVE_MVMNT_EVENT', :v_batch_id, 'ERROR', :v_start_time, :v_end_time,
            0, 0, 0, 0,
            :v_error_msg, CURRENT_TIMESTAMP()
        );
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.AWS.TASK_SP_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE CDC changes into data preservation table'
AS
    CALL D_RAW.AWS.SP_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT();

ALTER TASK D_RAW.AWS.TASK_SP_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT RESUME;

-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'EVENTS_LOCOMOTIVE_MVMNT_EVENT%' IN SCHEMA D_BRONZE.AWS;
-- SHOW STREAMS LIKE 'EVENTS_LOCOMOTIVE_MVMNT_EVENT%' IN SCHEMA D_RAW.AWS;
-- SHOW TASKS LIKE 'TASK_PROCESS_EVENTS_LOCOMOTIVE%' IN SCHEMA D_RAW.AWS;
-- CALL D_RAW.AWS.SP_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT();

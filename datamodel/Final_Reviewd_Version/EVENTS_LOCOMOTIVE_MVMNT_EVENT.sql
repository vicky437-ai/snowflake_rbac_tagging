...
Task         : D_RAW.AWS.TASK_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT
Primary Key  : event_id, event_timestamp_utc (COMPOSITE - 2 Columns)
Total Columns: 24 source + 6 CDC metadata = 30

NOTE: This table is from AWS schema (D_RAW.AWS)
      Column names are LOWERCASE with quotes (case-sensitive)
================================================================================
*/
...
-- =============================================================================
CREATE TABLE IF NOT EXISTS D_BRONZE.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT (
...
BEGIN
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        USING (
            ...
            FROM D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE src
            LEFT JOIN D_BRONZE.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT tgt 
                ON src."event_id" = tgt."event_id"
                AND src."event_timestamp_utc" = tgt."event_timestamp_utc"
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.AWS.TASK_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT
...
WHEN
    SYSTEM$STREAM_HAS_DATA('D_RAW.AWS.EVENTS_LOCOMOTIVE_MVMNT_EVENT_BASE_HIST_STREAM')
AS
    CALL D_RAW.AWS.SP_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT();

ALTER TASK D_RAW.AWS.TASK_PROCESS_EVENTS_LOCOMOTIVE_MVMNT_EVENT RESUME;

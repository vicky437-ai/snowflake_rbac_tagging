/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE
================================================================================
Source Table : D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE
Target Table : D_BRONZE.SADB.EQPMNT_MARK_OWNER_CLASS_CD
Stream       : D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE_HIST_STREAM
Procedure    : D_RAW.SADB.SP_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD()
Task         : D_RAW.SADB.TASK_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD
Primary Key  : MARK_OWNER_CLASS_CD (Single, VARCHAR(8))
Total Columns: 7 source + 3 SNW metadata + 6 CDC metadata = 16
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.EQPMNT_MARK_OWNER_CLASS_CD (
    MARK_OWNER_CLASS_CD VARCHAR(8) NOT NULL,
    RECORD_CREATE_TMS TIMESTAMP_NTZ(0),
    CREATE_USER_ID VARCHAR(32),
    RECORD_UPDATE_TMS TIMESTAMP_NTZ(0),
    UPDATE_USER_ID VARCHAR(32),
    DSCRPT_TXT VARCHAR(400),
    MARK_OWNER_CTGRY_CD VARCHAR(4),
    ...
    PRIMARY KEY (MARK_OWNER_CLASS_CD)
);

-- =============================================================================
-- STEP 2: Enable Change Tracking on Source Table
-- =============================================================================
ALTER TABLE D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE 
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE_HIST_STREAM
ON TABLE D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for EQPMNT_MARK_OWNER_CLASS_CD_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
        ...
        FROM D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE_HIST_STREAM
        WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.SADB.EQPMNT_MARK_OWNER_CLASS_CD AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE_HIST_STREAM src
            WHERE NVL(src.SNW_OPERATION_OWNER, '') NOT IN ('TSDPRG', 'EMEPRG')
        ) AS src
        ON tgt.MARK_OWNER_CLASS_CD = src.MARK_OWNER_CLASS_CD
        WHEN MATCHED THEN UPDATE SET
            ...
            tgt.MARK_OWNER_CTGRY_CD = src.MARK_OWNER_CTGRY_CD,
        ...
        WHEN NOT MATCHED THEN INSERT (
            MARK_OWNER_CLASS_CD, RECORD_CREATE_TMS, CREATE_USER_ID, RECORD_UPDATE_TMS, UPDATE_USER_ID,
            DSCRPT_TXT, MARK_OWNER_CTGRY_CD,
            ...
            src.MARK_OWNER_CLASS_CD, src.RECORD_CREATE_TMS, src.CREATE_USER_ID, src.RECORD_UPDATE_TMS, src.UPDATE_USER_ID,
            src.DSCRPT_TXT, src.MARK_OWNER_CTGRY_CD,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_EQPMNT_MARK_OWNER_CLASS_CD AS
    SELECT 
        MARK_OWNER_CLASS_CD, RECORD_CREATE_TMS, CREATE_USER_ID, RECORD_UPDATE_TMS, UPDATE_USER_ID,
        DSCRPT_TXT, MARK_OWNER_CTGRY_CD,
        SNW_OPERATION_TYPE, SNW_OPERATION_OWNER, SNW_LAST_REPLICATED,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_EQPMNT_MARK_OWNER_CLASS_CD;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_EQPMNT_MARK_OWNER_CLASS_CD;
    ...
    USING (
        SELECT 
            ...
            ROW_ID,
            :v_batch_id AS BATCH_ID
        FROM _CDC_STAGING_EQPMNT_MARK_OWNER_CLASS_CD
    ) AS src
    ON tgt.MARK_OWNER_CLASS_CD = src.MARK_OWNER_CLASS_CD
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process EQPMNT_MARK_OWNER_CLASS_CD_BASE CDC changes into data preservation table'
AS
    CALL D_RAW.SADB.SP_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD();

ALTER TASK D_RAW.SADB.TASK_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD RESUME;

-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'EQPMNT_MARK_OWNER_CLASS_CD%' IN SCHEMA D_BRONZE.SADB;
-- SHOW STREAMS LIKE 'EQPMNT_MARK_OWNER_CLASS_CD_BASE_HIST_STREAM%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD();

# Code Review: EQPMNT_MARK_OWNER_CLASS_CD CDC Data Preservation Script
**Review Date:** March 13, 2026  
**Reviewer:** Cortex Code  
**Script:** Scripts/DIM_EQUIP/EQPMNT_MARK_OWNER_CLASS_CD.sql  
...
| Item | Detail |
|------|--------|
| Source Table | `D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE` (10 columns) |
| Target Table | `D_BRONZE.SADB.EQPMNT_MARK_OWNER_CLASS_CD` (10 source + 6 CDC = 16 columns) |
| Stream | `D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE_HIST_STREAM` |
| Procedure | `D_RAW.SADB.SP_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD()` |
| Task | `D_RAW.SADB.TASK_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD` (5 min, no WHEN clause) |
...
|---|--------|--------------------------------|-----------|--------|
| 1 | Source table | `D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE` | `D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE` | DONE |
| 2 | Target table | `D_BRONZE.SADB.EQPMNT_POOL_ASGNMN` | `D_BRONZE.SADB.EQPMNT_MARK_OWNER_CLASS_CD` | DONE |
| 3 | Stream | `EQPMNT_POOL_ASGNMN_BASE_HIST_STREAM` | `EQPMNT_MARK_OWNER_CLASS_CD_BASE_HIST_STREAM` | DONE |
| 4 | Procedure | `SP_PROCESS_EQPMNT_POOL_ASGNMN()` | `SP_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD()` | DONE |
| 5 | Task | `TASK_PROCESS_EQPMNT_POOL_ASGNMN` | `TASK_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD` | DONE |
| 6 | Temp table | `_CDC_STAGING_EQPMNT_POOL_ASGNMN` | `_CDC_STAGING_EQPMNT_MARK_OWNER_CLASS_CD` | DONE |
| 7 | Primary key | `EQPMNT_POOL_ASGNMN_ID NUMBER(18,0)` | `MARK_OWNER_CLASS_CD VARCHAR(8)` | DONE |
...
Source `D_RAW.SADB.EQPMNT_MARK_OWNER_CLASS_CD_BASE` confirmed via `DESCRIBE TABLE` — **10 columns**.
...
|---|--------|-----------|---------|
| 1 | MARK_OWNER_CLASS_CD | VARCHAR(8) NOT NULL | YES (PK) |
...
| 6 | DSCRPT_TXT | VARCHAR(400) | YES |
| 7 | MARK_OWNER_CTGRY_CD | VARCHAR(4) | YES |
...
Verified: **zero** occurrences of old/reference names (`EQPMNT_POOL_ASGNMN`, `TRAIN_OPTRN_EVENT`, `OPTRN_EVENT`) in the script. All references use `EQPMNT_MARK_OWNER_CLASS_CD*` naming.
...
| Component | Result |
|-----------|--------|
| SP `SP_PROCESS_EQPMNT_MARK_OWNER_CLASS_CD()` | **COMPILED SUCCESSFULLY** |
...
3. **Info:** This table uses a **VARCHAR(8) primary key** (`MARK_OWNER_CLASS_CD`), unlike other DIM_EQUIP tables which use NUMBER(18,0). The MERGE ON clause correctly uses string comparison for this key.

---

## 11. Verdict

**APPROVED FOR PRODUCTION** — 100% column mapping (10/10 source + 6/6 CDC), all MERGE branches validated, primary key `MARK_OWNER_CLASS_CD` (VARCHAR(8)) correctly applied across all join clauses, dual-value filter (TSDPRG + EMEPRG) correctly applied with NULL-safe NVL handling, all object names follow naming convention, SP compiled successfully.

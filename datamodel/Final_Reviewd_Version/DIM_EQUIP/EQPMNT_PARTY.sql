/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.EQPMNT_PARTY_BASE
================================================================================
Source Table : D_RAW.SADB.EQPMNT_PARTY_BASE
Target Table : D_BRONZE.SADB.EQPMNT_PARTY
Stream       : D_RAW.SADB.EQPMNT_PARTY_BASE_HIST_STREAM
Procedure    : D_RAW.SADB.SP_PROCESS_EQPMNT_PARTY()
Task         : D_RAW.SADB.TASK_PROCESS_EQPMNT_PARTY
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.EQPMNT_PARTY (
    EQPMNT_PARTY_ID NUMBER(18,0) NOT NULL,
    RECORD_CREATE_TMS TIMESTAMP_NTZ(0),
    CREATE_USER_ID VARCHAR(32),
    RECORD_UPDATE_TMS TIMESTAMP_NTZ(0),
    UPDATE_USER_ID VARCHAR(32),
    EQPMNT_ID NUMBER(18,0),
    ENTITY_IDNTFR_CD VARCHAR(12),
    MARK_CD VARCHAR(16),
    ...
    PRIMARY KEY (EQPMNT_PARTY_ID)
);

-- =============================================================================
-- STEP 2: Enable Change Tracking on Source Table
-- =============================================================================
ALTER TABLE D_RAW.SADB.EQPMNT_PARTY_BASE 
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.EQPMNT_PARTY_BASE_HIST_STREAM
ON TABLE D_RAW.SADB.EQPMNT_PARTY_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for EQPMNT_PARTY_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_EQPMNT_PARTY()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
        ...
        FROM D_RAW.SADB.EQPMNT_PARTY_BASE_HIST_STREAM
        WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.SADB.EQPMNT_PARTY AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.SADB.EQPMNT_PARTY_BASE_HIST_STREAM src
            WHERE NVL(src.SNW_OPERATION_OWNER, '') NOT IN ('TSDPRG', 'EMEPRG')
        ) AS src
        ON tgt.EQPMNT_PARTY_ID = src.EQPMNT_PARTY_ID
        WHEN MATCHED THEN UPDATE SET
            ...
            tgt.ENTITY_IDNTFR_CD = src.ENTITY_IDNTFR_CD,
        ...
        WHEN NOT MATCHED THEN INSERT (
            EQPMNT_PARTY_ID, RECORD_CREATE_TMS, CREATE_USER_ID, RECORD_UPDATE_TMS, UPDATE_USER_ID,
            EQPMNT_ID, ENTITY_IDNTFR_CD, MARK_CD,
            ...
            src.EQPMNT_PARTY_ID, src.RECORD_CREATE_TMS, src.CREATE_USER_ID, src.RECORD_UPDATE_TMS, src.UPDATE_USER_ID,
            src.EQPMNT_ID, src.ENTITY_IDNTFR_CD, src.MARK_CD,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_EQPMNT_PARTY AS
    SELECT 
        EQPMNT_PARTY_ID, RECORD_CREATE_TMS, CREATE_USER_ID, RECORD_UPDATE_TMS, UPDATE_USER_ID,
        EQPMNT_ID, ENTITY_IDNTFR_CD, MARK_CD,
        SNW_OPERATION_TYPE, SNW_OPERATION_OWNER, SNW_LAST_REPLICATED,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_EQPMNT_PARTY;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_EQPMNT_PARTY;
    ...
    USING (
        SELECT 
            ...
            ROW_ID,
            :v_batch_id AS BATCH_ID
        FROM _CDC_STAGING_EQPMNT_PARTY
    ) AS src
    ON tgt.EQPMNT_PARTY_ID = src.EQPMNT_PARTY_ID
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_PROCESS_EQPMNT_PARTY
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process EQPMNT_PARTY_BASE CDC changes into data preservation table'
AS
    CALL D_RAW.SADB.SP_PROCESS_EQPMNT_PARTY();

ALTER TASK D_RAW.SADB.TASK_PROCESS_EQPMNT_PARTY RESUME;

-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'EQPMNT_PARTY%' IN SCHEMA D_BRONZE.SADB;
-- SHOW STREAMS LIKE 'EQPMNT_PARTY_BASE_HIST_STREAM%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_PROCESS_EQPMNT_PARTY%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_EQPMNT_PARTY();

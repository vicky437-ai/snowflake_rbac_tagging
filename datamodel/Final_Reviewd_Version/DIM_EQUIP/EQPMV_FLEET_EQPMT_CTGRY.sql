/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.EQPMV_FLEET_EQPMT_CTGRY_BASE
================================================================================
Source Table : D_RAW.SADB.EQPMV_FLEET_EQPMT_CTGRY_BASE
Target Table : D_BRONZE.SADB.EQPMV_FLEET_EQPMT_CTGRY
Stream       : D_RAW.SADB.EQPMV_FLEET_EQPMT_CTGRY_BASE_HIST_STREAM
Procedure    : D_RAW.SADB.SP_PROCESS_EQPMV_FLEET_EQPMT_CTGRY()
Task         : D_RAW.SADB.TASK_PROCESS_EQPMV_FLEET_EQPMT_CTGRY
Primary Key  : FLEET_EQPMT_CTGRY_ID (Single)
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.EQPMV_FLEET_EQPMT_CTGRY (
    FLEET_EQPMT_CTGRY_ID NUMBER(18,0) NOT NULL,
    PARTY_NM VARCHAR(400),
    FLEET_ID NUMBER(18,0),
    TARGET_EQPMT_CTGRY_ID NUMBER(18,0),
    ...
    PRIMARY KEY (FLEET_EQPMT_CTGRY_ID)
);

-- =============================================================================
-- STEP 2: Enable Change Tracking on Source Table
-- =============================================================================
ALTER TABLE D_RAW.SADB.EQPMV_FLEET_EQPMT_CTGRY_BASE 
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.EQPMV_FLEET_EQPMT_CTGRY_BASE_HIST_STREAM
ON TABLE D_RAW.SADB.EQPMV_FLEET_EQPMT_CTGRY_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for EQPMV_FLEET_EQPMT_CTGRY_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_EQPMV_FLEET_EQPMT_CTGRY()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
        ...
        FROM D_RAW.SADB.EQPMV_FLEET_EQPMT_CTGRY_BASE_HIST_STREAM
        WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.SADB.EQPMV_FLEET_EQPMT_CTGRY AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.SADB.EQPMV_FLEET_EQPMT_CTGRY_BASE_HIST_STREAM src
            WHERE NVL(src.SNW_OPERATION_OWNER, '') NOT IN ('TSDPRG', 'EMEPRG')
        ) AS src
        ON tgt.FLEET_EQPMT_CTGRY_ID = src.FLEET_EQPMT_CTGRY_ID
        WHEN MATCHED THEN UPDATE SET
            tgt.PARTY_NM = src.PARTY_NM,
            tgt.FLEET_ID = src.FLEET_ID,
            tgt.TARGET_EQPMT_CTGRY_ID = src.TARGET_EQPMT_CTGRY_ID,
        ...
        WHEN NOT MATCHED THEN INSERT (
            FLEET_EQPMT_CTGRY_ID, PARTY_NM, FLEET_ID, TARGET_EQPMT_CTGRY_ID,
            ...
            src.FLEET_EQPMT_CTGRY_ID, src.PARTY_NM, src.FLEET_ID, src.TARGET_EQPMT_CTGRY_ID,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_EQPMV_FLEET_EQPMT_CTGRY AS
    SELECT 
        FLEET_EQPMT_CTGRY_ID, PARTY_NM, FLEET_ID, TARGET_EQPMT_CTGRY_ID,
        CREATE_USER_ID, RECORD_CREATE_TMS, RECORD_UPDATE_TMS, UPDATE_USER_ID,
        SOURCE_CREATE_TMS, SOURCE_UPDATE_TMS,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_EQPMV_FLEET_EQPMT_CTGRY;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_EQPMV_FLEET_EQPMT_CTGRY;
    ...
    USING (
        SELECT 
            ...
            ROW_ID,
            :v_batch_id AS BATCH_ID
        FROM _CDC_STAGING_EQPMV_FLEET_EQPMT_CTGRY
    ) AS src
    ON tgt.FLEET_EQPMT_CTGRY_ID = src.FLEET_EQPMT_CTGRY_ID
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_PROCESS_EQPMV_FLEET_EQPMT_CTGRY
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process EQPMV_FLEET_EQPMT_CTGRY_BASE CDC changes into data preservation table'
AS
    CALL D_RAW.SADB.SP_PROCESS_EQPMV_FLEET_EQPMT_CTGRY();

ALTER TASK D_RAW.SADB.TASK_PROCESS_EQPMV_FLEET_EQPMT_CTGRY RESUME;

-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'EQPMV_FLEET_EQPMT_CTGRY%' IN SCHEMA D_BRONZE.SADB;
-- SHOW STREAMS LIKE 'EQPMV_FLEET_EQPMT_CTGRY_BASE_HIST_STREAM%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_PROCESS_EQPMV_FLEET_EQPMT_CTGRY%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_EQPMV_FLEET_EQPMT_CTGRY();

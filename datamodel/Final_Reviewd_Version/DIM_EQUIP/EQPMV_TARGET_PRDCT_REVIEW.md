# Code Review: EQPMV_TARGET_PRDCT CDC Data Preservation Script
**Review Date:** March 13, 2026  
**Reviewer:** Cortex Code  
**Script:** Scripts/DIM_EQUIP/EQPMV_TARGET_PRDCT.sql  
...
| Item | Detail |
|------|--------|
| Source Table | `D_RAW.SADB.EQPMV_TARGET_PRDCT_BASE` (12 columns) |
| Target Table | `D_BRONZE.SADB.EQPMV_TARGET_PRDCT` (12 source + 6 CDC = 18 columns) |
| Stream | `D_RAW.SADB.EQPMV_TARGET_PRDCT_BASE_HIST_STREAM` |
| Procedure | `D_RAW.SADB.SP_PROCESS_EQPMV_TARGET_PRDCT()` |
| Task | `D_RAW.SADB.TASK_PROCESS_EQPMV_TARGET_PRDCT` (5 min, no WHEN clause) |
...
|---|--------|--------------------------------|-----------|--------|
| 1 | Source table | `D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE` | `D_RAW.SADB.EQPMV_TARGET_PRDCT_BASE` | DONE |
| 2 | Target table | `D_BRONZE.SADB.EQPMNT_POOL_ASGNMN` | `D_BRONZE.SADB.EQPMV_TARGET_PRDCT` | DONE |
| 3 | Stream | `EQPMNT_POOL_ASGNMN_BASE_HIST_STREAM` | `EQPMV_TARGET_PRDCT_BASE_HIST_STREAM` | DONE |
| 4 | Procedure | `SP_PROCESS_EQPMNT_POOL_ASGNMN()` | `SP_PROCESS_EQPMV_TARGET_PRDCT()` | DONE |
| 5 | Task | `TASK_PROCESS_EQPMNT_POOL_ASGNMN` | `TASK_PROCESS_EQPMV_TARGET_PRDCT` | DONE |
| 6 | Temp table | `_CDC_STAGING_EQPMNT_POOL_ASGNMN` | `_CDC_STAGING_EQPMV_TARGET_PRDCT` | DONE |
| 7 | Primary key | `EQPMNT_POOL_ASGNMN_ID` | `TARGET_PRDCT_ID` | DONE |
...
Source `D_RAW.SADB.EQPMV_TARGET_PRDCT_BASE` confirmed via `DESCRIBE TABLE` — **12 columns**.
...
|---|--------|-----------|---------|
| 1 | TARGET_PRDCT_ID | NUMBER(18,0) NOT NULL | YES (PK) |
| 2 | PARTY_NM | VARCHAR(400) | YES |
| 3 | PRDCT_NM | VARCHAR(120) | YES |
...
Verified: **zero** occurrences of old/reference names (`EQPMNT_POOL_ASGNMN`, `TRAIN_OPTRN_EVENT`, `OPTRN_EVENT`) in the script. All references use `EQPMV_TARGET_PRDCT*` naming.
...
| Component | Result |
|-----------|--------|
| SP `SP_PROCESS_EQPMV_TARGET_PRDCT()` | **COMPILED SUCCESSFULLY** |
...
3. **Info:** `CREATE_USER_ID` and `UPDATE_USER_ID` are VARCHAR(1020), matching source exactly. `PRDCT_NM` is VARCHAR(120), unique to this table.

---

## 11. Verdict

**APPROVED FOR PRODUCTION** — 100% column mapping (12/12 source + 6/6 CDC), all MERGE branches validated, primary key `TARGET_PRDCT_ID` correctly applied across all join clauses, dual-value filter (TSDPRG + EMEPRG) correctly applied with NULL-safe NVL handling, all object names follow naming convention, SP compiled successfully.

/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.EQPMV_FLEET_POOL_BASE
================================================================================
Source Table : D_RAW.SADB.EQPMV_FLEET_POOL_BASE
Target Table : D_BRONZE.SADB.EQPMV_FLEET_POOL
Stream       : D_RAW.SADB.EQPMV_FLEET_POOL_BASE_HIST_STREAM
Procedure    : D_RAW.SADB.SP_PROCESS_EQPMV_FLEET_POOL()
Task         : D_RAW.SADB.TASK_PROCESS_EQPMV_FLEET_POOL
Primary Key  : FLEET_POOL_ID (Single)
Total Columns: 11 source + 3 SNW metadata + 6 CDC metadata = 20
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.EQPMV_FLEET_POOL (
    FLEET_POOL_ID NUMBER(18,0) NOT NULL,
    PARTY_NM VARCHAR(400),
    FLEET_ID NUMBER(18,0),
    EQPMNT_POOL_ID NUMBER(18,0),
    FLEET_SBGRP_ID NUMBER(18,0),
    ...
    PRIMARY KEY (FLEET_POOL_ID)
);

-- =============================================================================
-- STEP 2: Enable Change Tracking on Source Table
-- =============================================================================
ALTER TABLE D_RAW.SADB.EQPMV_FLEET_POOL_BASE 
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.EQPMV_FLEET_POOL_BASE_HIST_STREAM
ON TABLE D_RAW.SADB.EQPMV_FLEET_POOL_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for EQPMV_FLEET_POOL_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_EQPMV_FLEET_POOL()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
        ...
        FROM D_RAW.SADB.EQPMV_FLEET_POOL_BASE_HIST_STREAM
        WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.SADB.EQPMV_FLEET_POOL AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.SADB.EQPMV_FLEET_POOL_BASE_HIST_STREAM src
            WHERE NVL(src.SNW_OPERATION_OWNER, '') NOT IN ('TSDPRG', 'EMEPRG')
        ) AS src
        ON tgt.FLEET_POOL_ID = src.FLEET_POOL_ID
        WHEN MATCHED THEN UPDATE SET
            tgt.PARTY_NM = src.PARTY_NM,
            tgt.FLEET_ID = src.FLEET_ID,
            tgt.EQPMNT_POOL_ID = src.EQPMNT_POOL_ID,
            tgt.FLEET_SBGRP_ID = src.FLEET_SBGRP_ID,
        ...
        WHEN NOT MATCHED THEN INSERT (
            FLEET_POOL_ID, PARTY_NM, FLEET_ID, EQPMNT_POOL_ID, FLEET_SBGRP_ID,
            ...
            src.FLEET_POOL_ID, src.PARTY_NM, src.FLEET_ID, src.EQPMNT_POOL_ID, src.FLEET_SBGRP_ID,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_EQPMV_FLEET_POOL AS
    SELECT 
        FLEET_POOL_ID, PARTY_NM, FLEET_ID, EQPMNT_POOL_ID, FLEET_SBGRP_ID,
        CREATE_USER_ID, RECORD_CREATE_TMS, RECORD_UPDATE_TMS, UPDATE_USER_ID,
        SOURCE_CREATE_TMS, SOURCE_UPDATE_TMS,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_EQPMV_FLEET_POOL;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_EQPMV_FLEET_POOL;
    ...
    USING (
        SELECT 
            ...
            ROW_ID,
            :v_batch_id AS BATCH_ID
        FROM _CDC_STAGING_EQPMV_FLEET_POOL
    ) AS src
    ON tgt.FLEET_POOL_ID = src.FLEET_POOL_ID
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_PROCESS_EQPMV_FLEET_POOL
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process EQPMV_FLEET_POOL_BASE CDC changes into data preservation table'
AS
    CALL D_RAW.SADB.SP_PROCESS_EQPMV_FLEET_POOL();

ALTER TASK D_RAW.SADB.TASK_PROCESS_EQPMV_FLEET_POOL RESUME;

-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'EQPMV_FLEET_POOL%' IN SCHEMA D_BRONZE.SADB;
-- SHOW STREAMS LIKE 'EQPMV_FLEET_POOL%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_PROCESS_EQPMV_FLEET_POOL%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_EQPMV_FLEET_POOL();

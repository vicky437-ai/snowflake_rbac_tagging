...
-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing (ENHANCED v2.0 - Production)
-- =============================================================================
...
DECLARE
    ...
    v_rows_duplicate_resolved NUMBER DEFAULT 0;
    v_result VARCHAR;
    v_error_msg VARCHAR;
    v_start_time TIMESTAMP_NTZ;
    v_end_time TIMESTAMP_NTZ;
    v_execution_status VARCHAR DEFAULT 'SUCCESS';
    v_base_table_check NUMBER DEFAULT 0;
    v_sqlcode_captured NUMBER;
    v_sqlstate_captured VARCHAR;
BEGIN
    v_batch_id := 'BATCH_' || TO_VARCHAR(CURRENT_TIMESTAMP(), 'YYYYMMDD_HH24MISS');
    v_start_time := CURRENT_TIMESTAMP();
    
    -- =========================================================================
    -- ENSURE MONITORING TABLE EXISTS (Fix #6 - Preventive)
    -- =========================================================================
    ...
    -- =========================================================================
    -- CHECK 1: Detect if stream is stale (FIX #2 - More Specific Error Detection)
    -- =========================================================================
    BEGIN
    ...
    EXCEPTION
        WHEN STATEMENT_ERROR THEN
            v_sqlcode_captured := SQLCODE;
            v_error_msg := SQLERRM;
            
            -- Only treat as stale stream if error indicates staleness/invalidation
            -- Snowflake stream errors typically contain 'invalid' or specific codes
            IF (v_error_msg ILIKE '%invalid%' OR v_error_msg ILIKE '%stale%' 
                OR v_sqlcode_captured IN (2000, 2003, 2043)) THEN
                v_stream_stale := TRUE;
            ELSE
                -- Re-raise if it's not a staleness issue (network, permissions, etc.)
                RAISE;
            END IF;
    END;
    
    -- =========================================================================
    -- RECOVERY: If stream is stale, recreate it (FIX #5 - Validate Base Table)
    -- =========================================================================
    IF (v_stream_stale = TRUE) THEN
        ...
        -- FIX #5: Validate base table exists and is accessible BEFORE recreating stream
        BEGIN
            SELECT COUNT(*) INTO v_base_table_check 
            FROM D_RAW.EHMS.DTQ_DTCTD_EQPMNT_BASE 
            LIMIT 1;
        EXCEPTION
            WHEN STATEMENT_ERROR THEN
                v_end_time := CURRENT_TIMESTAMP();
                v_error_msg := 'RECOVERY_FAILED: Base table validation failed - ' || SQLERRM || ' (Code: ' || SQLCODE || ')';
                INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG 
                    (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, 
                     ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT) 
                VALUES ('DTQ_DTCTD_EQPMNT', :v_batch_id, 'RECOVERY_FAILED', :v_start_time, :v_end_time, 
                        0, 0, 0, 0, :v_error_msg, CURRENT_TIMESTAMP());
                RETURN v_error_msg;
        END;
        
        -- Base table is valid, safe to recreate stream
        CREATE OR REPLACE STREAM D_RAW.EHMS.DTQ_DTCTD_EQPMNT_BASE_HIST_STREAM
        ON TABLE D_RAW.EHMS.DTQ_DTCTD_EQPMNT_BASE
        SHOW_INITIAL_ROWS = TRUE
        COMMENT = 'CDC Stream recreated after staleness detection at ' || CURRENT_TIMESTAMP()::VARCHAR;
        
        -- Use staging pattern for consistency (Fix #7 - same pattern as normal processing)
        ...
        SELECT 
            ...
            FALSE AS CDC_IS_UPDATE,
            NULL AS ROW_ID
        FROM D_RAW.EHMS.DTQ_DTCTD_EQPMNT_BASE_HIST_STREAM;
        
        -- Recovery MERGE using staging table
        ...
        -- FIX #4: Capture SQLROWCOUNT immediately after MERGE
        LET v_rows_merged := SQLROWCOUNT;
        LET v_end_time := CURRENT_TIMESTAMP();
        
        -- FIX #3: Cleanup temp table BEFORE logging (transaction safety)
        DROP TABLE IF EXISTS _CDC_STAGING_DTQ_DTCTD_EQPMNT;
        
        -- FIX #3: Log to monitoring (if this fails, exception handler will catch it)
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG 
            (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, 
             ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT) 
        VALUES ('DTQ_DTCTD_EQPMNT', :v_batch_id, 'RECOVERY', :v_start_time, :v_end_time, 
    ...
    IF (v_staging_count = 0) THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG 
            (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, 
             ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT) 
        VALUES ('DTQ_DTCTD_EQPMNT', :v_batch_id, 'NO_DATA', :v_start_time, :v_end_time, 
    ...
    -- =========================================================================
    -- PRE-MERGE METRICS (FIX #8 - Track duplicate insert resolution)
    -- =========================================================================
    SELECT 
    ...
    -- FIX #8: Detect potential duplicate insert scenarios (INSERT matching existing PK)
    -- This can happen due to out-of-order CDC events or source system duplicates
    SELECT COUNT(DISTINCT stg.DTCTD_EQPMNT_ID)
    INTO v_rows_duplicate_resolved
    FROM _CDC_STAGING_DTQ_DTCTD_EQPMNT stg
    INNER JOIN D_BRONZE.EHMS.DTQ_DTCTD_EQPMNT tgt 
        ON stg.DTCTD_EQPMNT_ID = tgt.DTCTD_EQPMNT_ID
    WHERE stg.CDC_ACTION = 'INSERT' AND stg.CDC_IS_UPDATE = FALSE;
    ...
    -- FIX #8: DUPLICATE INSERT RESOLUTION
    -- Occurs when CDC_ACTION='INSERT' but record already exists (out-of-order events or duplicate PK)
    -- Treat as UPDATE to preserve data integrity
    WHEN MATCHED AND src.CDC_ACTION = 'INSERT' AND src.CDC_IS_UPDATE = FALSE THEN
        UPDATE SET
            ...
            tgt.SNW_OPERATION_OWNER = src.SNW_OPERATION_OWNER,
            tgt.SNW_LAST_REPLICATED = src.SNW_LAST_REPLICATED,
            tgt.CDC_OPERATION = 'DUPLICATE_RESOLVED', -- FIX #8: Special marker for troubleshooting
            tgt.CDC_TIMESTAMP = CURRENT_TIMESTAMP(),
            tgt.IS_DELETED = FALSE,
            tgt.RECORD_UPDATED_AT = CURRENT_TIMESTAMP(),
            tgt.SOURCE_LOAD_BATCH_ID = src.BATCH_ID
    ...
    -- FIX #4: Capture SQLROWCOUNT immediately after MERGE (critical timing)
    LET v_rows_merged := SQLROWCOUNT;
    LET v_end_time := CURRENT_TIMESTAMP();
    
    -- FIX #3: Clean up temp table BEFORE logging (ensures cleanup even if logging fails)
    DROP TABLE IF EXISTS _CDC_STAGING_DTQ_DTCTD_EQPMNT;
    
    -- FIX #3: Log success with all metrics (if this INSERT fails, goes to exception handler)
    INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG 
        (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, 
         ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT, ADDITIONAL_METRICS) 
    VALUES ('DTQ_DTCTD_EQPMNT', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time, 
            :v_rows_merged, :v_rows_inserted, :v_rows_updated, :v_rows_deleted, NULL, CURRENT_TIMESTAMP(),
    ...
    RETURN 'SUCCESS: Processed ' || v_rows_merged || ' CDC changes (I:' || v_rows_inserted 
           || ' U:' || v_rows_updated || ' D:' || v_rows_deleted 
           || ' DUP:' || v_rows_duplicate_resolved || '). Batch: ' || v_batch_id;

-- FIX #1: Enhanced Exception Handling with Specific Handlers
EXCEPTION
    WHEN STATEMENT_ERROR THEN
        LET v_end_time := CURRENT_TIMESTAMP();
        LET v_sqlcode_captured := SQLCODE;
        LET v_sqlstate_captured := SQLSTATE;
        LET v_error_msg := 'STATEMENT_ERROR: ' || SQLERRM || ' (Code: ' || v_sqlcode_captured 
                          || ', State: ' || v_sqlstate_captured || ')';
        
        -- Ensure temp table cleanup
        DROP TABLE IF EXISTS _CDC_STAGING_DTQ_DTCTD_EQPMNT;
        ...
        BEGIN
            INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG 
                (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, 
                 ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT) 
            VALUES ('DTQ_DTCTD_EQPMNT', :v_batch_id, 'SQL_ERROR', :v_start_time, :v_end_time, 
                    0, 0, 0, 0, :v_error_msg, CURRENT_TIMESTAMP());
        EXCEPTION
            WHEN OTHER THEN
                -- If logging fails, at least return error message
                NULL;
        END;
        
        RETURN v_error_msg || ' at ' || v_end_time::VARCHAR;
    
    WHEN OTHER THEN
        LET v_end_time := CURRENT_TIMESTAMP();
        LET v_sqlcode_captured := SQLCODE;
        LET v_sqlstate_captured := SQLSTATE;
        LET v_error_msg := 'UNEXPECTED_ERROR: ' || SQLERRM || ' (Code: ' || v_sqlcode_captured 
                          || ', State: ' || v_sqlstate_captured || ')';
        ...
        BEGIN
            INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG 
                (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, 
                 ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT) 
            VALUES ('DTQ_DTCTD_EQPMNT', :v_batch_id, 'UNKNOWN_ERROR', :v_start_time, :v_end_time, 
                    0, 0, 0, 0, :v_error_msg, CURRENT_TIMESTAMP());
        EXCEPTION
            WHEN OTHER THEN
                -- If logging fails, at least return error message

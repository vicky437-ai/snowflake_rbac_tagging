/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE
================================================================================
Source Table : D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE
Target Table : D_BRONZE.SADB.EQPMNT_POOL_ASGNMN
Stream       : D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE_HIST_STREAM
Procedure    : D_RAW.SADB.SP_PROCESS_EQPMNT_POOL_ASGNMN()
Task         : D_RAW.SADB.TASK_PROCESS_EQPMNT_POOL_ASGNMN
Primary Key  : EQPMNT_POOL_ASGNMN_ID (Single)
Total Columns: 14 source + 3 SNW metadata + 6 CDC metadata = 23
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.EQPMNT_POOL_ASGNMN (
    EQPMNT_POOL_ASGNMN_ID NUMBER(18,0) NOT NULL,
    RECORD_CREATE_TMS TIMESTAMP_NTZ(0),
    CREATE_USER_ID VARCHAR(32),
    RECORD_UPDATE_TMS TIMESTAMP_NTZ(0),
    UPDATE_USER_ID VARCHAR(32),
    EQPMNT_ID NUMBER(18,0),
    EQPMNT_POOL_ID NUMBER(18,0),
    EQPMNT_POOL_ASGNMN_STATUS_CD VARCHAR(60),
    ...
    PRIMARY KEY (EQPMNT_POOL_ASGNMN_ID)
);

-- =============================================================================
-- STEP 2: Enable Change Tracking on Source Table
-- =============================================================================
ALTER TABLE D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE 
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE_HIST_STREAM
ON TABLE D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for EQPMNT_POOL_ASGNMN_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_EQPMNT_POOL_ASGNMN()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
        ...
        FROM D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE_HIST_STREAM
        WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.SADB.EQPMNT_POOL_ASGNMN AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE_HIST_STREAM src
            WHERE NVL(src.SNW_OPERATION_OWNER, '') NOT IN ('TSDPRG', 'EMEPRG')
        ) AS src
        ON tgt.EQPMNT_POOL_ASGNMN_ID = src.EQPMNT_POOL_ASGNMN_ID
        WHEN MATCHED THEN UPDATE SET
            ...
            tgt.EQPMNT_ID = src.EQPMNT_ID,
            tgt.EQPMNT_POOL_ID = src.EQPMNT_POOL_ID,
            tgt.EQPMNT_POOL_ASGNMN_STATUS_CD = src.EQPMNT_POOL_ASGNMN_STATUS_CD,
        ...
        WHEN NOT MATCHED THEN INSERT (
            EQPMNT_POOL_ASGNMN_ID, RECORD_CREATE_TMS, CREATE_USER_ID, RECORD_UPDATE_TMS, UPDATE_USER_ID,
            EQPMNT_ID, EQPMNT_POOL_ID, EQPMNT_POOL_ASGNMN_STATUS_CD,
            ...
            src.EQPMNT_POOL_ASGNMN_ID, src.RECORD_CREATE_TMS, src.CREATE_USER_ID, src.RECORD_UPDATE_TMS, src.UPDATE_USER_ID,
            src.EQPMNT_ID, src.EQPMNT_POOL_ID, src.EQPMNT_POOL_ASGNMN_STATUS_CD,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_EQPMNT_POOL_ASGNMN AS
    SELECT 
        EQPMNT_POOL_ASGNMN_ID, RECORD_CREATE_TMS, CREATE_USER_ID, RECORD_UPDATE_TMS, UPDATE_USER_ID,
        EQPMNT_ID, EQPMNT_POOL_ID, EQPMNT_POOL_ASGNMN_STATUS_CD,
        CREATE_PARTY_RLTNSH_USER_ID, SOURCE_CREATE_TMS, UPDATE_PARTY_RLTNSH_USER_ID, SOURCE_UPDATE_TMS,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_EQPMNT_POOL_ASGNMN;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_EQPMNT_POOL_ASGNMN;
    ...
    USING (
        SELECT 
            ...
            ROW_ID,
            :v_batch_id AS BATCH_ID
        FROM _CDC_STAGING_EQPMNT_POOL_ASGNMN
    ) AS src
    ON tgt.EQPMNT_POOL_ASGNMN_ID = src.EQPMNT_POOL_ASGNMN_ID
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_PROCESS_EQPMNT_POOL_ASGNMN
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process EQPMNT_POOL_ASGNMN_BASE CDC changes into data preservation table'
AS
    CALL D_RAW.SADB.SP_PROCESS_EQPMNT_POOL_ASGNMN();

ALTER TASK D_RAW.SADB.TASK_PROCESS_EQPMNT_POOL_ASGNMN RESUME;

-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'EQPMNT_POOL_ASGNMN%' IN SCHEMA D_BRONZE.SADB;
-- SHOW STREAMS LIKE 'EQPMNT_POOL_ASGNMN%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_PROCESS_EQPMNT_POOL_ASGNMN%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_EQPMNT_POOL_ASGNMN();

/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.CSTMR_RGSTRD_MARK_JN_GRP_BASE
================================================================================
Source Table : D_RAW.SADB.CSTMR_RGSTRD_MARK_JN_GRP_BASE
Target Table : D_BRONZE.SADB.CSTMR_RGSTRD_MARK_JN_GRP
Stream       : D_RAW.SADB.CSTMR_RGSTRD_MARK_JN_GRP_BASE_HIST_STREAM
Procedure    : D_RAW.SADB.SP_PROCESS_CSTMR_RGSTRD_MARK_JN_GRP()
Task         : D_RAW.SADB.TASK_PROCESS_CSTMR_RGSTRD_MARK_JN_GRP
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.CSTMR_RGSTRD_MARK_JN_GRP (
    DATA_SOURCE_CD VARCHAR(40) NOT NULL,
    GROUP_ROLE_CD VARCHAR(80) NOT NULL,
    ...
    PRIMARY KEY (DATA_SOURCE_CD, GROUP_ROLE_CD, MARK_CD)
);

-- =============================================================================
-- STEP 2: Enable Change Tracking on Source Table
-- =============================================================================
ALTER TABLE D_RAW.SADB.CSTMR_RGSTRD_MARK_JN_GRP_BASE 
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.CSTMR_RGSTRD_MARK_JN_GRP_BASE_HIST_STREAM
ON TABLE D_RAW.SADB.CSTMR_RGSTRD_MARK_JN_GRP_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for CSTMR_RGSTRD_MARK_JN_GRP_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_CSTMR_RGSTRD_MARK_JN_GRP()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
        ...
        FROM D_RAW.SADB.CSTMR_RGSTRD_MARK_JN_GRP_BASE_HIST_STREAM
        WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.SADB.CSTMR_RGSTRD_MARK_JN_GRP AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.SADB.CSTMR_RGSTRD_MARK_JN_GRP_BASE_HIST_STREAM src
            WHERE NVL(src.SNW_OPERATION_OWNER, '') NOT IN ('TSDPRG', 'EMEPRG')
        ) AS src
        ON tgt.DATA_SOURCE_CD = src.DATA_SOURCE_CD
           AND tgt.GROUP_ROLE_CD = src.GROUP_ROLE_CD
           AND tgt.MARK_CD = src.MARK_CD
        ...
        WHEN NOT MATCHED THEN INSERT (
            DATA_SOURCE_CD, GROUP_ROLE_CD, MARK_CD,
            RECORD_CREATE_TMS, CREATE_USER_ID, RECORD_UPDATE_TMS, UPDATE_USER_ID,
            ...
            src.DATA_SOURCE_CD, src.GROUP_ROLE_CD, src.MARK_CD,
            src.RECORD_CREATE_TMS, src.CREATE_USER_ID, src.RECORD_UPDATE_TMS, src.UPDATE_USER_ID,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_CSTMR_RGSTRD_MARK_JN_GRP AS
    SELECT 
        DATA_SOURCE_CD, GROUP_ROLE_CD, MARK_CD,
        RECORD_CREATE_TMS, CREATE_USER_ID, RECORD_UPDATE_TMS, UPDATE_USER_ID,
        SNW_OPERATION_TYPE, SNW_OPERATION_OWNER, SNW_LAST_REPLICATED,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_CSTMR_RGSTRD_MARK_JN_GRP;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_CSTMR_RGSTRD_MARK_JN_GRP;
    ...
    USING (
        SELECT 
            ...
            ROW_ID,
            :v_batch_id AS BATCH_ID
        FROM _CDC_STAGING_CSTMR_RGSTRD_MARK_JN_GRP
    ) AS src
    ON tgt.DATA_SOURCE_CD = src.DATA_SOURCE_CD
       AND tgt.GROUP_ROLE_CD = src.GROUP_ROLE_CD
       AND tgt.MARK_CD = src.MARK_CD
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_PROCESS_CSTMR_RGSTRD_MARK_JN_GRP
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process CSTMR_RGSTRD_MARK_JN_GRP_BASE CDC changes into data preservation table'
AS
    CALL D_RAW.SADB.SP_PROCESS_CSTMR_RGSTRD_MARK_JN_GRP();

ALTER TASK D_RAW.SADB.TASK_PROCESS_CSTMR_RGSTRD_MARK_JN_GRP RESUME;

-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'CSTMR_RGSTRD_MARK_JN_GRP%' IN SCHEMA D_BRONZE.SADB;
-- SHOW STREAMS LIKE 'CSTMR_RGSTRD_MARK_JN_GRP_BASE_HIST_STREAM%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_PROCESS_CSTMR_RGSTRD_MARK_JN_GRP%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_CSTMR_RGSTRD_MARK_JN_GRP();

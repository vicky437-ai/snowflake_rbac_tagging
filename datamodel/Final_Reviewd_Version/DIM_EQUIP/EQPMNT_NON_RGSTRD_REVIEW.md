# Code Review: EQPMNT_NON_RGSTRD CDC Data Preservation Script
**Review Date:** March 13, 2026  
**Reviewer:** Cortex Code  
**Script:** Scripts/DIM_EQUIP/EQPMNT_NON_RGSTRD.sql  
...
| Item | Detail |
|------|--------|
| Source Table | `D_RAW.SADB.EQPMNT_NON_RGSTRD_BASE` (11 columns) |
| Target Table | `D_BRONZE.SADB.EQPMNT_NON_RGSTRD` (11 source + 6 CDC = 17 columns) |
| Stream | `D_RAW.SADB.EQPMNT_NON_RGSTRD_BASE_HIST_STREAM` |
| Procedure | `D_RAW.SADB.SP_PROCESS_EQPMNT_NON_RGSTRD()` |
| Task | `D_RAW.SADB.TASK_PROCESS_EQPMNT_NON_RGSTRD` (5 min, no WHEN clause) |
...
|---|--------|--------------------------------|-----------|--------|
| 1 | Source table | `D_RAW.SADB.EQPMNT_POOL_ASGNMN_BASE` | `D_RAW.SADB.EQPMNT_NON_RGSTRD_BASE` | DONE |
| 2 | Target table | `D_BRONZE.SADB.EQPMNT_POOL_ASGNMN` | `D_BRONZE.SADB.EQPMNT_NON_RGSTRD` | DONE |
| 3 | Stream | `EQPMNT_POOL_ASGNMN_BASE_HIST_STREAM` | `EQPMNT_NON_RGSTRD_BASE_HIST_STREAM` | DONE |
| 4 | Procedure | `SP_PROCESS_EQPMNT_POOL_ASGNMN()` | `SP_PROCESS_EQPMNT_NON_RGSTRD()` | DONE |
| 5 | Task | `TASK_PROCESS_EQPMNT_POOL_ASGNMN` | `TASK_PROCESS_EQPMNT_NON_RGSTRD` | DONE |
| 6 | Temp table | `_CDC_STAGING_EQPMNT_POOL_ASGNMN` | `_CDC_STAGING_EQPMNT_NON_RGSTRD` | DONE |
| 7 | Primary key | `EQPMNT_POOL_ASGNMN_ID` | `EQPMNT_ID` | DONE |
...
Source `D_RAW.SADB.EQPMNT_NON_RGSTRD_BASE` confirmed via `DESCRIBE TABLE` — **11 columns**.
...
| 8 | CHECK_DIGIT_NBR | VARCHAR(8) | YES |
...
**Data impact (current):**
- 7,846,348 NULL rows — INCLUDED (correct)
- 2 EDMGR rows — INCLUDED (correct, not in filter list)
...
Verified: **zero** occurrences of old/reference names (`EQPMNT_POOL_ASGNMN`, `TRAIN_OPTRN_EVENT`, `OPTRN_EVENT`) in the script. All references use `EQPMNT_NON_RGSTRD*` naming.
...
| Component | Result |
|-----------|--------|
| SP `SP_PROCESS_EQPMNT_NON_RGSTRD()` | **COMPILED SUCCESSFULLY** |
...
3. **Info:** This is the **largest table** in the DIM_EQUIP batch at 7.8M rows. Initial load via SHOW_INITIAL_ROWS will process all rows on first run. Monitor task execution time and warehouse sizing.

---

## 11. Verdict

**APPROVED FOR PRODUCTION** — 100% column mapping (11/11 source + 6/6 CDC), all MERGE branches validated, primary key `EQPMNT_ID` correctly applied across all join clauses, dual-value filter (TSDPRG + EMEPRG) correctly applied with NULL-safe NVL handling, all object names follow naming convention, SP compiled successfully.

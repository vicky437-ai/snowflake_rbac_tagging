...
Task         : D_RAW.SADB.TASK_SP_PROCESS_LCMTV_EMIS
Primary Key  : COMPOSITE (MARK_CD, EQPUN_NBR) - 2 columns
Total Columns: 85 source + 6 CDC metadata = 91
Filter       : SNW_OPERATION_OWNER NOT IN ('TSDPRG','EMEPRG') (exclude purged records)
-- =============================================================================
*/
-- STEP 1: Create Target Data Preservation Table
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.LCMTV_EMIS (
...
COMMENT = 'Bronze data preservation layer for Locomotive Emission Specifications.
Source: Oracle SADB (LCMTV_EMIS) replicated via IDMC CDC into D_RAW.SADB.LCMTV_EMIS_BASE.
Data Source Tables: D_RAW.SADB.LCMTV_EMIS_BASE (85 source columns + 3 SNW metadata).
...
Pipeline Objects: Stream D_RAW.SADB.LCMTV_EMIS_BASE_HIST_STREAM | Procedure D_RAW.SADB.SP_PROCESS_LCMTV_EMIS() | Task D_RAW.SADB.TASK_SP_PROCESS_LCMTV_EMIS.
Refresh Frequency: Every 5 minutes via Snowflake Task (incremental CDC MERGE).
Primary Key: MARK_CD, EQPUN_NBR (Composite).';
...
BEGIN
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        USING (
            ...
            FROM D_RAW.SADB.LCMTV_EMIS_BASE_HIST_STREAM src
        ...
        WHEN NOT MATCHED THEN INSERT (
            ...
            CODED_CAB_SIGNAL_CD, LCMTV_STRTR_TYPE_CD, SNW_OPERATION_TYPE, SNW_OPERATION_OWNER, SNW_LAST_REPLICATED,
            ...
            src.CODED_CAB_SIGNAL_CD, src.LCMTV_STRTR_TYPE_CD, src.SNW_OPERATION_TYPE, src.SNW_OPERATION_OWNER, src.SNW_LAST_REPLICATED,
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'LCMTV_EMIS', :v_batch_id, 'RECOVERY', :v_start_time, :v_end_time,
    ...
    IF (v_staging_count = 0) THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'LCMTV_EMIS', :v_batch_id, 'NO_DATA', :v_start_time, :v_end_time,
            0, 0, 0, 0,
...
EXCEPTION
    WHEN OTHER THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'LCMTV_EMIS', :v_batch_id, 'ERROR', :v_start_time, :v_end_time,
            0, 0, 0, 0,
            :v_error_msg, CURRENT_TIMESTAMP()
        );
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_SP_PROCESS_LCMTV_EMIS
...
ALTER TASK D_RAW.SADB.TASK_SP_PROCESS_LCMTV_EMIS RESUME;
...
-- SHOW TASKS LIKE 'TASK_SP_PROCESS_LCMTV_EMIS%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_LCMTV_EMIS();

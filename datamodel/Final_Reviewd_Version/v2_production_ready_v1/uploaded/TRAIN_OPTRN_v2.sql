/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.TRAIN_OPTRN_BASE
================================================================================
Source Table : D_RAW.SADB.TRAIN_OPTRN_BASE
Target Table : D_BRONZE.SADB.TRAIN_OPTRN
Stream       : D_RAW.SADB.TRAIN_OPTRN_BASE_HIST_STREAM
Procedure    : D_RAW.SADB.SP_PROCESS_TRAIN_OPTRN()
Task         : D_RAW.SADB.TASK_SP_PROCESS_TRAIN_OPTRN
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.TRAIN_OPTRN (
...
COMMENT = 'Bronze data preservation layer for Train Operations.
Source: Oracle SADB (TRAIN_OPTRN) replicated via IDMC CDC into D_RAW.SADB.TRAIN_OPTRN_BASE.
Data Source Tables: D_RAW.SADB.TRAIN_OPTRN_BASE (18 source columns + 3 SNW metadata).
...
Pipeline Objects: Stream D_RAW.SADB.TRAIN_OPTRN_BASE_HIST_STREAM | Procedure D_RAW.SADB.SP_PROCESS_TRAIN_OPTRN() | Task D_RAW.SADB.TASK_SP_PROCESS_TRAIN_OPTRN.
...
-- =============================================================================
ALTER TABLE D_RAW.SADB.TRAIN_OPTRN_BASE
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.TRAIN_OPTRN_BASE_HIST_STREAM
ON TABLE D_RAW.SADB.TRAIN_OPTRN_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for TRAIN_OPTRN_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_TRAIN_OPTRN()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
            ...
            FROM D_RAW.SADB.TRAIN_OPTRN_BASE_HIST_STREAM
            WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.SADB.TRAIN_OPTRN AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.SADB.TRAIN_OPTRN_BASE_HIST_STREAM src
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRAIN_OPTRN', :v_batch_id, 'RECOVERY', :v_start_time, :v_end_time,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_TRAIN_OPTRN AS
    SELECT 
        OPTRN_ID, TRAIN_TYPE_CD, TRAIN_KIND_CD, MTP_OPTRN_PRFL_NM, SCHDLD_TRAIN_TYPE_CD,
        OPTRN_NM, TRAIN_PRTY_NBR, TRAIN_RATING_CD, VRNC_IND,
        RECORD_CREATE_TMS, RECORD_UPDATE_TMS, CREATE_USER_ID, UPDATE_USER_ID,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_TRAIN_OPTRN;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_TRAIN_OPTRN;
        v_end_time := CURRENT_TIMESTAMP();
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRAIN_OPTRN', :v_batch_id, 'NO_DATA', :v_start_time, :v_end_time,
            0, 0, 0, 0,
    ...
    INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
        ...
        'TRAIN_OPTRN', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time,
...
EXCEPTION
    WHEN OTHER THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRAIN_OPTRN', :v_batch_id, 'ERROR', :v_start_time, :v_end_time,
            0, 0, 0, 0,
            :v_error_msg, CURRENT_TIMESTAMP()
        );
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_SP_PROCESS_TRAIN_OPTRN
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process TRAIN_OPTRN_BASE CDC changes into data preservation table'
AS
    CALL D_RAW.SADB.SP_PROCESS_TRAIN_OPTRN();

ALTER TASK D_RAW.SADB.TASK_SP_PROCESS_TRAIN_OPTRN RESUME;


-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'TRAIN_OPTRN%' IN SCHEMA D_BRONZE.SADB;
-- SHOW STREAMS LIKE 'TRAIN_OPTRN_BASE%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_SP_PROCESS_TRAIN_OPTRN%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_TRAIN_OPTRN();

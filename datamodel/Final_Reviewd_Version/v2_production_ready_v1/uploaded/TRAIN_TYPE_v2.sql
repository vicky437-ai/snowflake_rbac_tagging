/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.TRAIN_TYPE_BASE
================================================================================
Source Table : D_RAW.SADB.TRAIN_TYPE_BASE
Target Table : D_BRONZE.SADB.TRAIN_TYPE
Stream       : D_RAW.SADB.TRAIN_TYPE_BASE_HIST_STREAM
Procedure    : D_RAW.SADB.SP_PROCESS_TRAIN_TYPE()
Task         : D_RAW.SADB.TASK_SP_PROCESS_TRAIN_TYPE
Primary Key  : TRAIN_TYPE_CD (Single)
Total Columns: 9 source + 6 CDC metadata = 15
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.TRAIN_TYPE (
    TRAIN_TYPE_CD VARCHAR(16) NOT NULL,
    DSCRPT_TXT VARCHAR(240),
...
COMMENT = 'Bronze data preservation layer for Train Type Reference.
Source: Oracle SADB (TRAIN_TYPE) replicated via IDMC CDC into D_RAW.SADB.TRAIN_TYPE_BASE.
Data Source Tables: D_RAW.SADB.TRAIN_TYPE_BASE (9 source columns + 3 SNW metadata).
...
Pipeline Objects: Stream D_RAW.SADB.TRAIN_TYPE_BASE_HIST_STREAM | Procedure D_RAW.SADB.SP_PROCESS_TRAIN_TYPE() | Task D_RAW.SADB.TASK_SP_PROCESS_TRAIN_TYPE.
...
-- =============================================================================
ALTER TABLE D_RAW.SADB.TRAIN_TYPE_BASE 
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.TRAIN_TYPE_BASE_HIST_STREAM
ON TABLE D_RAW.SADB.TRAIN_TYPE_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for TRAIN_TYPE_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing 
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_TRAIN_TYPE()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
            ...
            FROM D_RAW.SADB.TRAIN_TYPE_BASE_HIST_STREAM
            WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.SADB.TRAIN_TYPE AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.SADB.TRAIN_TYPE_BASE_HIST_STREAM src
            WHERE NVL(src.SNW_OPERATION_OWNER, '') NOT IN ('TSDPRG', 'EMEPRG')
        ) AS src
        ON tgt.TRAIN_TYPE_CD = src.TRAIN_TYPE_CD
        ...
        WHEN NOT MATCHED THEN INSERT (
            TRAIN_TYPE_CD, DSCRPT_TXT, RECORD_CREATE_TMS, RECORD_UPDATE_TMS,
            CREATE_USER_ID, UPDATE_USER_ID, SNW_OPERATION_TYPE, SNW_OPERATION_OWNER, SNW_LAST_REPLICATED,
            CDC_OPERATION, CDC_TIMESTAMP, IS_DELETED,
            RECORD_CREATED_AT, RECORD_UPDATED_AT, SOURCE_LOAD_BATCH_ID
        ) VALUES (
            src.TRAIN_TYPE_CD, src.DSCRPT_TXT, src.RECORD_CREATE_TMS, src.RECORD_UPDATE_TMS,
            src.CREATE_USER_ID, src.UPDATE_USER_ID, src.SNW_OPERATION_TYPE, src.SNW_OPERATION_OWNER, src.SNW_LAST_REPLICATED,
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRAIN_TYPE', :v_batch_id, 'RECOVERY', :v_start_time, :v_end_time,
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_TRAIN_TYPE AS
    SELECT 
        TRAIN_TYPE_CD, DSCRPT_TXT, RECORD_CREATE_TMS, RECORD_UPDATE_TMS,
        CREATE_USER_ID, UPDATE_USER_ID, SNW_OPERATION_TYPE, SNW_OPERATION_OWNER, SNW_LAST_REPLICATED,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_TRAIN_TYPE;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_TRAIN_TYPE;
        v_end_time := CURRENT_TIMESTAMP();
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRAIN_TYPE', :v_batch_id, 'NO_DATA', :v_start_time, :v_end_time,
            0, 0, 0, 0,
    ...
    INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
        ...
        'TRAIN_TYPE', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time,
...
EXCEPTION
    WHEN OTHER THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRAIN_TYPE', :v_batch_id, 'ERROR', :v_start_time, :v_end_time,
            0, 0, 0, 0,
            :v_error_msg, CURRENT_TIMESTAMP()
        );
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_SP_PROCESS_TRAIN_TYPE
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process TRAIN_TYPE_BASE CDC changes into data preservation table '
AS
    CALL D_RAW.SADB.SP_PROCESS_TRAIN_TYPE();

ALTER TASK D_RAW.SADB.TASK_SP_PROCESS_TRAIN_TYPE RESUME;


-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'TRAIN_TYPE%' IN SCHEMA D_BRONZE.SADB;
-- SHOW STREAMS LIKE 'TRAIN_TYPE%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_SP_PROCESS_TRAIN_TYPE%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_TRAIN_TYPE();

/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.SADB.TRKFC_TRSTN_BASE
================================================================================
...
Task         : D_RAW.SADB.TASK_SP_PROCESS_TRKFC_TRSTN
Primary Key  : COMPOSITE (SCAC_CD, FSAC_CD) - 2 columns
Total Columns: 41 source + 6 CDC metadata = 47
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.TRKFC_TRSTN (
...
COMMENT = 'Bronze data preservation layer for Track Configuration Transitions.
Source: Oracle SADB (TRKFC_TRSTN) replicated via IDMC CDC into D_RAW.SADB.TRKFC_TRSTN_BASE.
Data Source Tables: D_RAW.SADB.TRKFC_TRSTN_BASE (41 source columns + 3 SNW metadata).
...
Pipeline Objects: Stream D_RAW.SADB.TRKFC_TRSTN_BASE_HIST_STREAM | Procedure D_RAW.SADB.SP_PROCESS_TRKFC_TRSTN() | Task D_RAW.SADB.TASK_SP_PROCESS_TRKFC_TRSTN.
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.SADB.TRKFC_TRSTN_BASE_HIST_STREAM
...
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.SADB.SP_PROCESS_TRKFC_TRSTN()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
            ...
            FROM D_RAW.SADB.TRKFC_TRSTN_BASE_HIST_STREAM
            WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.SADB.TRKFC_TRSTN AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.SADB.TRKFC_TRSTN_BASE_HIST_STREAM src
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRKFC_TRSTN', :v_batch_id, 'RECOVERY', :v_start_time, :v_end_time,
    ...
    IF (v_staging_count = 0) THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRKFC_TRSTN', :v_batch_id, 'NO_DATA', :v_start_time, :v_end_time,
            0, 0, 0, 0,
    ...
    INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
        ...
        'TRKFC_TRSTN', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time,
...
EXCEPTION
    WHEN OTHER THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRKFC_TRSTN', :v_batch_id, 'ERROR', :v_start_time, :v_end_time,
            0, 0, 0, 0,
            :v_error_msg, CURRENT_TIMESTAMP()
        );
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_SP_PROCESS_TRKFC_TRSTN
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process TRKFC_TRSTN_BASE CDC changes into data preservation table '
AS
    CALL D_RAW.SADB.SP_PROCESS_TRKFC_TRSTN();

ALTER TASK D_RAW.SADB.TASK_SP_PROCESS_TRKFC_TRSTN RESUME;
...
-- SHOW STREAMS LIKE 'TRKFC_TRSTN%' IN SCHEMA D_RAW.SADB;
-- SHOW TASKS LIKE 'TASK_SP_PROCESS_TRKFC_TRSTN%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_TRKFC_TRSTN();

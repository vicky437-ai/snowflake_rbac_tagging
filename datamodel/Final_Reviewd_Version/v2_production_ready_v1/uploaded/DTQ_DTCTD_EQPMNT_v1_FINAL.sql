...
BEGIN
    ...
    -- =========================================================================
    -- ENSURE MONITORING TABLE EXISTS ( Preventive)
    -- =========================================================================
    ...
    -- =========================================================================
    -- CHECK 1: Detect if stream is stale ( More Specific Error Detection)
    -- =========================================================================
    BEGIN
    ...
    EXCEPTION
        WHEN STATEMENT_ERROR THEN
            ...
            IF (v_error_msg ILIKE '%invalid%' OR v_error_msg ILIKE '%stale%'
                OR v_error_msg ILIKE '%dropped%' OR v_error_msg ILIKE '%does not exist%'
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        -- Recovery staging with DEDUPLICATION + NULL PK filter
        ...
        SELECT * FROM (
            ...
            FROM D_RAW.EHMS.DTQ_DTCTD_EQPMNT_BASE_HIST_STREAM
            WHERE DTCTD_EQPMNT_ID IS NOT NULL
        ) WHERE DEDUP_RN = 1;
    ...
    -- =========================================================================
    -- Normal mode staging CTAS with DEDUPLICATION + NULL PK filter
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_DTQ_DTCTD_EQPMNT AS
    SELECT * FROM (
        SELECT 
            DTCTD_EQPMNT_ID, CREATE_USER_ID, RECORD_CREATE_TMS, RECORD_UPDATE_TMS, UPDATE_USER_ID,
    ...
    SELECT 'DTQ_DTCTD_EQPMNT', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time, 
...
EXCEPTION
    WHEN STATEMENT_ERROR THEN
        v_end_time := CURRENT_TIMESTAMP();
        v_sqlcode_captured := SQLCODE;
        v_sqlstate_captured := SQLSTATE;
        v_error_msg := 'STATEMENT_ERROR: ' || SQLERRM || ' (Code: ' || v_sqlcode_captured 
    ...
    WHEN OTHER THEN
        ...
        v_sqlcode_captured := SQLCODE;
        v_sqlstate_captured := SQLSTATE;
        v_error_msg := 'UNEXPECTED_ERROR: ' || SQLERRM || ' (Code: ' || v_sqlcode_captured 
                          || ', State: ' || v_sqlstate_captured || ')';

/*
================================================================================
CDC PIPELINE MONITORING AND OBSERVABILITY FRAMEWORK - VERSION 4.0
================================================================================
Version:        4.0.0 (PRODUCTION READY - Rewrite 2026-03-25)
Created:        2026-02-25
Updated:        2026-03-25
...
CHANGES FROM V3.0:
    ...
    3. SP_CAPTURE_STREAM_HEALTH rewritten:
       - Uses INFORMATION_SCHEMA.STREAMS (direct metadata)
       - Eliminates SHOW STREAMS + RESULT_SCAN dependency
       - Properly populates IS_STALE, STALE_AFTER, HOURS_UNTIL_STALE
    4. SP_CAPTURE_TASK_HEALTH rewritten:
...
-- =============================================================================
-- STEP 8: CREATE SP - CAPTURE STREAM HEALTH (INFORMATION_SCHEMA - NO RESULT_SCAN)
-- =============================================================================
-- v4.0 FIX: Replaced SHOW STREAMS + RESULT_SCAN(LAST_QUERY_ID()) with direct
-- INFORMATION_SCHEMA.STREAMS query. The v3.0 SP had a bug where FETCH INTO
-- v_is_stale was getting the wrong column (created_on instead of stale flag),
-- and the rs2 RESULTSET was declared but never used to populate the INSERT.
-- =============================================================================
...
BEGIN
    ...
    FOR rec IN c1 DO
        ...
        BEGIN
            -- ================================================================
            -- Direct metadata query: INFORMATION_SCHEMA.STREAMS
            -- No SHOW STREAMS, no RESULT_SCAN, no LAST_QUERY_ID dependency
            -- ================================================================
            SELECT 
                TRUE,
                STALE::BOOLEAN,
            ...
            WHERE STREAM_CATALOG = 'D_RAW'
              ...
              AND STREAM_NAME = :v_stream_short
            LIMIT 1;
...
-- =============================================================================
-- STEP 10: CREATE SP - CAPTURE DATA QUALITY METRICS (PERCENTAGE-BASED THRESHOLDS)
-- =============================================================================
-- v4.0 FIX: DQ thresholds changed from absolute row diff (10/100) to
-- percentage-based (>=99% HEALTHY, >=95% WARNING, <95% CRITICAL).
-- This handles tables of vastly different sizes correctly:
--   TRAIN_KIND (17 cols, ~100 rows) vs STNWYB_MSG_DN (137 cols, millions)
-- =============================================================================
...
BEGIN
    ...
    FOR rec IN c1 DO
        ...
        BEGIN
            ...
            -- Latest timestamps for lag calculation
...
BEGIN
    ...
    -- Alert 3: Data quality issues (WARNING/CRITICAL based on DQ status)
    INSERT INTO D_BRONZE.MONITORING.CDC_ALERT_LOG (ALERT_TYPE, ALERT_SEVERITY, TABLE_NAME, ALERT_MESSAGE, ALERT_DETAILS)
    SELECT 
        'DATA_QUALITY', 
        CASE WHEN DATA_QUALITY_STATUS = 'CRITICAL' THEN 'CRITICAL' ELSE 'WARNING' END, 
        TABLE_NAME,
        'Data quality issue: ' || TABLE_NAME || ' (match_pct=' || ROW_COUNT_MATCH_PCT || '%, source_filtered=' || SOURCE_FILTERED_COUNT || ', target_active=' || TARGET_ACTIVE_COUNT || ', diff=' || ROW_COUNT_DIFF || ')',
        OBJECT_CONSTRUCT('source_total', SOURCE_ROW_COUNT, 'source_filtered', SOURCE_FILTERED_COUNT, 'target_total', TARGET_ROW_COUNT, 'target_active', TARGET_ACTIVE_COUNT, 'diff', ROW_COUNT_DIFF, 'match_pct', ROW_COUNT_MATCH_PCT)
...
-- SELECT * FROM D_BRONZE.MONITORING.VW_PIPELINE_TREND_7D;
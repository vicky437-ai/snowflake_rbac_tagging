# Code Review: TRKFCG_FXPLA_TRACK_LCTN_DN CDC Data Preservation Script
**Review Date:** March 11, 2026 (Rev 2 - Bug Fix updates)  
**Reviewer:** Cortex Code  
**Script:** Scripts/Bug_Fix_2026_03_05/TRKFCG_FXPLA_TRACK_LCTN_DN.sql  
...
| Item | Detail |
|------|--------|
| Source Table | `D_RAW.SADB.TRKFCG_FXPLA_TRACK_LCTN_DN_BASE` (57 columns) |
| Target Table | `D_BRONZE.SADB.TRKFCG_FXPLA_TRACK_LCTN_DN` (57 source + 6 CDC = 63 columns) |
| Stream | `D_RAW.SADB.TRKFCG_FXPLA_TRACK_LCTN_DN_BASE_HIST_STREAM` |
| Procedure | `D_RAW.SADB.SP_PROCESS_TRKFCG_FXPLA_TRACK_LCTN_DN()` |
| Task | `D_RAW.SADB.TASK_PROCESS_TRKFCG_FXPLA_TRACK_LCTN_DN` (5 min, no WHEN clause) |
...
| 2 | New column | N/A (56 source) | `SNW_OPERATION_OWNER VARCHAR(256)` added (57 source) | DONE |
...
| 7 | Column count | 56 source + 6 CDC = 62 | 57 source + 6 CDC = 63 | DONE |

---

## 3. Column Mapping (Source -> Target) -- 57/57 = 100%

| # | Column | Data Type | Mapped? |
|---|--------|-----------|---------|
| 1 | GRPHC_OBJECT_VRSN_ID | NUMBER(18,0) NOT NULL | YES (PK) |
| 2-54 | VRSN_CREATE_TMS through PSTN_MODE_CD | Various | YES (all 53) |
| 55 | SNW_OPERATION_TYPE | VARCHAR(1) | YES |
| 56 | SNW_OPERATION_OWNER | VARCHAR(256) | YES (NEW) |
| 57 | SNW_LAST_REPLICATED | TIMESTAMP_NTZ(9) | YES |
...
|--------|-------------|---------------------|----------|--------|
| Recovery UPDATE | 55/55 | YES | 5/5 | PASS |
| Recovery INSERT | 63 cols = 63 vals | YES | 6/6 | PASS |
| Main UPDATE | 55/55 | YES | 5/5 | PASS |
| Main DELETE | Soft-delete only | N/A | 5/5 | PASS |
| Main RE-INSERT | 55/55 | YES | 5/5 | PASS |
| Main NEW INSERT | 63 cols = 63 vals | YES | 6/6 | PASS |
...
**Data impact:** 344,101 NULL rows included. 0 TSDPRG/EMEPRG -- filter is preventive.
...
| Component | Result |
|-----------|--------|
| SP `SP_PROCESS_TRKFCG_FXPLA_TRACK_LCTN_DN()` | **COMPILED SUCCESSFULLY** |
...
|----------|-------|-----|-------|
| Column Mapping | 10 | 10 | 57/57 source + 6/6 CDC = 100% |
...
1. **Info:** No TSDPRG/EMEPRG rows in source (344,101 rows, all NULL owner) -- filter is preventive.

---

## 9. Verdict

**APPROVED FOR PRODUCTION** -- 100% column mapping (57/57 source + 6/6 CDC = 63 total), all MERGE branches validated, dual-value filter correctly applied, stream-based recovery implemented, SP compiled successfully.

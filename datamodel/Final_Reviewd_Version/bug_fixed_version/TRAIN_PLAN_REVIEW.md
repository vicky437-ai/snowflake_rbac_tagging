# Code Review: TRAIN_PLAN CDC Data Preservation Script
**Review Date:** March 11, 2026 (Rev 2 - Bug Fix updates)  
**Reviewer:** Cortex Code  
**Script:** Scripts/Bug_Fix_2026_03_05/TRAIN_PLAN.sql  
...
| Item | Detail |
|------|--------|
| Source Table | `D_RAW.SADB.TRAIN_PLAN_BASE` (18 columns) |
| Target Table | `D_BRONZE.SADB.TRAIN_PLAN` (18 source + 6 CDC = 24 columns) |
| Stream | `D_RAW.SADB.TRAIN_PLAN_BASE_HIST_STREAM` |
| Procedure | `D_RAW.SADB.SP_PROCESS_TRAIN_PLAN()` |
| Task | `D_RAW.SADB.TASK_PROCESS_TRAIN_PLAN` (5 min, no WHEN clause) |
...
| 2 | New column | N/A (17 source) | `SNW_OPERATION_OWNER VARCHAR(256)` added (18 source) | DONE |
...
| 7 | Column count | 17 source + 6 CDC = 23 | 18 source + 6 CDC = 24 | DONE |
...
|---|--------|-----------|---------|
| 1 | TRAIN_PLAN_ID | NUMBER(18,0) NOT NULL | YES (PK) |
...
| 6 | TRAIN_PLAN_NM | VARCHAR(32) | YES |
...
| 14 | TENANT_SCAC_CD | VARCHAR(16) | YES |
| 15 | NAMING_OPTION_CD | VARCHAR(8) | YES |
...
**Data impact:** 53,209 NULL rows included, **5,698 TSDPRG rows excluded**. 0 EMEPRG (preventive).
...
| Component | Result |
|-----------|--------|
| SP `SP_PROCESS_TRAIN_PLAN()` | **COMPILED SUCCESSFULLY** |
...
1. **Info:** 5,698 TSDPRG rows will be actively filtered. 0 EMEPRG (preventive).

---

## 9. Verdict

**APPROVED FOR PRODUCTION** -- 100% column mapping (18/18 source + 6/6 CDC = 24 total), all MERGE branches validated, dual-value filter correctly applied (5,698 TSDPRG rows excluded), stream-based recovery implemented, SP compiled successfully.

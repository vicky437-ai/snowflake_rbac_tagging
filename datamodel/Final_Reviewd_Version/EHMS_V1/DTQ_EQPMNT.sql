/*
================================================================================
DATA PRESERVATION SCRIPT FOR D_RAW.EHMS.DTQ_EQPMNT_BASE
================================================================================
Source Table : D_RAW.EHMS.DTQ_EQPMNT_BASE
Target Table : D_BRONZE.EHMS.DTQ_EQPMNT
Stream       : D_RAW.EHMS.DTQ_EQPMNT_BASE_HIST_STREAM
Procedure    : D_RAW.EHMS.SP_PROCESS_DTQ_EQPMNT()
Task         : D_RAW.EHMS.TASK_SP_PROCESS_DTQ_EQPMNT
...
CHANGES      :
  ...
  - Renamed objects: EHMSAPP_ prefix removed (EHMSAPP_DTQ_EQPMNT -> DTQ_EQPMNT) including source table
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.EHMS.DTQ_EQPMNT (
...
COMMENT = 'Bronze data preservation layer for EHMS Equipment.
Source: Oracle EHMS (DTQ_EQPMNT) replicated via IDMC CDC into D_RAW.EHMS.DTQ_EQPMNT_BASE.
Data Source Tables: D_RAW.EHMS.DTQ_EQPMNT_BASE (16 source columns + 3 SNW metadata).
...
Pipeline Objects: Stream D_RAW.EHMS.DTQ_EQPMNT_BASE_HIST_STREAM | Procedure D_RAW.EHMS.SP_PROCESS_DTQ_EQPMNT() | Task D_RAW.EHMS.TASK_SP_PROCESS_DTQ_EQPMNT.
...
-- =============================================================================
ALTER TABLE D_RAW.EHMS.DTQ_EQPMNT_BASE 
...
-- =============================================================================
CREATE OR REPLACE STREAM D_RAW.EHMS.DTQ_EQPMNT_BASE_HIST_STREAM
ON TABLE D_RAW.EHMS.DTQ_EQPMNT_BASE
SHOW_INITIAL_ROWS = TRUE
COMMENT = 'CDC Stream for DTQ_EQPMNT_BASE data preservation. SHOW_INITIAL_ROWS=TRUE for initial load.';

-- =============================================================================
-- STEP 4: Create Stored Procedure for CDC Processing (ENHANCED v1)
-- =============================================================================
CREATE OR REPLACE PROCEDURE D_RAW.EHMS.SP_PROCESS_DTQ_EQPMNT()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    BEGIN
        ...
        FROM D_RAW.EHMS.DTQ_EQPMNT_BASE_HIST_STREAM
        WHERE 1=0;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        MERGE INTO D_BRONZE.EHMS.DTQ_EQPMNT AS tgt
        USING (
            SELECT 
                src.*,
                'INSERT' AS CDC_OP,
                :v_batch_id AS BATCH_ID
            FROM D_RAW.EHMS.DTQ_EQPMNT_BASE_HIST_STREAM src
        ) AS src
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT) VALUES ('DTQ_EQPMNT', :v_batch_id, 'RECOVERY', :v_start_time, :v_end_time, :v_rows_merged, :v_rows_merged, 0, 0, :v_error_msg, CURRENT_TIMESTAMP());
    ...
    -- =========================================================================
    CREATE OR REPLACE TEMPORARY TABLE _CDC_STAGING_DTQ_EQPMNT AS
    SELECT 
        EQPMNT_ID, CREATE_USER_ID, RECORD_CREATE_TMS, RECORD_UPDATE_TMS, UPDATE_USER_ID,
        MARK_CD, EQPUN_NBR, AAR_CAR_CD, AXLE_QTY, OTSD_LENGTH_QTY,
        PRIOR_EQPUN_NBR, PRIOR_MARK_CD, TRUCK_CENTER_LENGTH_INCH_QTY,
    ...
    SELECT COUNT(*) INTO v_staging_count FROM _CDC_STAGING_DTQ_EQPMNT;
    
    IF (v_staging_count = 0) THEN
        DROP TABLE IF EXISTS _CDC_STAGING_DTQ_EQPMNT;
        v_end_time := CURRENT_TIMESTAMP();
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT) VALUES ('DTQ_EQPMNT', :v_batch_id, 'NO_DATA', :v_start_time, :v_end_time, 0, 0, 0, 0, NULL, CURRENT_TIMESTAMP());
    ...
    INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT) VALUES ('DTQ_EQPMNT', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time, :v_rows_merged, :v_rows_inserted, :v_rows_updated, :v_rows_deleted, NULL, CURRENT_TIMESTAMP());
...
EXCEPTION
    WHEN OTHER THEN
        ...
        v_error_msg := SQLERRM;
        DROP TABLE IF EXISTS _CDC_STAGING_DTQ_EQPMNT;
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (TABLE_NAME, BATCH_ID, EXECUTION_STATUS, START_TIME, END_TIME, ROWS_PROCESSED, ROWS_INSERTED, ROWS_UPDATED, ROWS_DELETED, ERROR_MESSAGE, CREATED_AT) VALUES ('DTQ_EQPMNT', :v_batch_id, 'ERROR', :v_start_time, :v_end_time, 0, 0, 0, 0, :v_error_msg, CURRENT_TIMESTAMP());
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.EHMS.TASK_SP_PROCESS_DTQ_EQPMNT
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process DTQ_EQPMNT_BASE CDC changes into data preservation table'
AS
    CALL D_RAW.EHMS.SP_PROCESS_DTQ_EQPMNT();

ALTER TASK D_RAW.EHMS.TASK_SP_PROCESS_DTQ_EQPMNT RESUME;

-- =============================================================================
-- VERIFICATION QUERIES
-- =============================================================================
-- SHOW TABLES LIKE 'DTQ_EQPMNT%' IN SCHEMA D_BRONZE.EHMS;
-- SHOW STREAMS LIKE 'DTQ_EQPMNT%' IN SCHEMA D_RAW.EHMS;
-- SHOW TASKS LIKE 'TASK_SP_PROCESS_DTQ_EQPMNT%' IN SCHEMA D_RAW.EHMS;
-- CALL D_RAW.EHMS.SP_PROCESS_DTQ_EQPMNT();

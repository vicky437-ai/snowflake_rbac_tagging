# EQPMV_EQPMT_EVENT_TYPE v1 Enhancement Review

**Document Date:** March 13, 2026  
**Original Version:** EQPMV_EQPMT_EVENT_TYPE.sql  
**Enhanced Version:** EQPMV_EQPMT_EVENT_TYPE_v1.sql  
...
This document details the two critical enhancements implemented in `EQPMV_EQPMT_EVENT_TYPE_v1.sql` based on production readiness review findings.
...
```sql
BEGIN
    SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.EQPMV_EQPMT_EVENT_TYPE_BASE_HIST_STREAM') 
    INTO v_stale_after;
...
WHERE TABLE_NAME = 'EQPMV_EQPMT_EVENT_TYPE' 
...
3. **Verify**: Run `CALL D_RAW.SADB.SP_PROCESS_EQPMV_EQPMT_EVENT_TYPE()` and check log entry

# LCMTV_MVMNT_EVENT v1 Enhancement Review

**Document Date:** March 13, 2026  
**Original Version:** LCMTV_MVMNT_EVENT.sql  
**Enhanced Version:** LCMTV_MVMNT_EVENT_v1.sql  
...
This document details the two critical enhancements implemented in `LCMTV_MVMNT_EVENT_v1.sql` based on production readiness review findings.
...
```sql
BEGIN
    SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.LCMTV_MVMNT_EVENT_BASE_HIST_STREAM') 
    INTO v_stale_after;
...
WHERE TABLE_NAME = 'LCMTV_MVMNT_EVENT' 
...
3. **Verify**: Run `CALL D_RAW.SADB.SP_PROCESS_LCMTV_MVMNT_EVENT()` and check log entry

# TRKFCG_SBDVSN v1 Enhancement Review

**Document Date:** March 13, 2026  
**Original Version:** TRKFCG_SBDVSN.sql  
**Enhanced Version:** TRKFCG_SBDVSN_v1.sql  
...
This document details the two critical enhancements implemented in `TRKFCG_SBDVSN_v1.sql` based on production readiness review findings.
...
```sql
BEGIN
    SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.TRKFCG_SBDVSN_BASE_HIST_STREAM') 
    INTO v_stale_after;
...
WHERE TABLE_NAME = 'TRKFCG_SBDVSN' 
...
3. **Verify**: Run `CALL D_RAW.SADB.SP_PROCESS_TRKFCG_SBDVSN()` and check log entry

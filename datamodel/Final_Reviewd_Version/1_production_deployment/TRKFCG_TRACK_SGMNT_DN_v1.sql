...
Total Columns: 59 source + 6 CDC metadata = 65
...
-- =============================================================================
CREATE OR ALTER TABLE D_BRONZE.SADB.TRKFCG_TRACK_SGMNT_DN (
...
BEGIN
    ...
    -- =========================================================================
    BEGIN
        SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.TRKFCG_TRACK_SGMNT_DN_BASE_HIST_STREAM') INTO v_stale_after;
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        USING (
            ...
            FROM D_RAW.SADB.TRKFCG_TRACK_SGMNT_DN_BASE_HIST_STREAM src
        ...
        WHEN NOT MATCHED THEN INSERT (
            ...
            OPRTNG_CPCTY_CARS_QTY, SNW_OPERATION_TYPE, SNW_OPERATION_OWNER, SNW_LAST_REPLICATED,
            ...
            src.OPRTNG_CPCTY_CARS_QTY, src.SNW_OPERATION_TYPE, src.SNW_OPERATION_OWNER, src.SNW_LAST_REPLICATED,
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRKFCG_TRACK_SGMNT_DN', :v_batch_id, 'RECOVERY', :v_start_time, :v_end_time,
    ...
    IF (v_staging_count = 0) THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRKFCG_TRACK_SGMNT_DN', :v_batch_id, 'NO_DATA', :v_start_time, :v_end_time,
            0, 0, 0, 0,
    ...
    INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
        ...
        'TRKFCG_TRACK_SGMNT_DN', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time,
...
EXCEPTION
    WHEN OTHER THEN
        ...
        INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
            ...
            'TRKFCG_TRACK_SGMNT_DN', :v_batch_id, 'ERROR', :v_start_time, :v_end_time,
            0, 0, 0, 0,
            :v_error_msg, CURRENT_TIMESTAMP()
        );
...
CREATE OR REPLACE TASK D_RAW.SADB.TASK_PROCESS_TRKFCG_TRACK_SGMNT_DN
    ...
    COMMENT = 'Task to process TRKFCG_TRACK_SGMNT_DN_BASE CDC changes into data preservation table (v1 - enhanced logging)'
AS
...
-- CALL D_RAW.SADB.SP_PROCESS_TRKFCG_TRACK_SGMNT_DN();
-- SELECT * FROM D_BRONZE.MONITORING.CDC_EXECUTION_LOG WHERE TABLE_NAME = 'TRKFCG_TRACK_SGMNT_DN' ORDER BY CREATED_AT DESC LIMIT 10;

# TRKFCG_FXPLA_TRACK_LCTN_DN v1 Enhancement Review

**Document Date:** March 13, 2026  
**Original Version:** TRKFCG_FXPLA_TRACK_LCTN_DN.sql  
**Enhanced Version:** TRKFCG_FXPLA_TRACK_LCTN_DN_v1.sql  
...
This document details the two critical enhancements implemented in `TRKFCG_FXPLA_TRACK_LCTN_DN_v1.sql` based on production readiness review findings.
...
```sql
BEGIN
    SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.TRKFCG_FXPLA_TRACK_LCTN_DN_BASE_HIST_STREAM') 
    INTO v_stale_after;
...
WHERE TABLE_NAME = 'TRKFCG_FXPLA_TRACK_LCTN_DN' 
...
3. **Verify**: Run `CALL D_RAW.SADB.SP_PROCESS_TRKFCG_FXPLA_TRACK_LCTN_DN()` and check log entry

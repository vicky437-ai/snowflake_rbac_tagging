# TRAIN_OPTRN_LEG v1 Enhancement Review

**Document Date:** March 13, 2026  
**Original Version:** TRAIN_OPTRN_LEG.sql  
**Enhanced Version:** TRAIN_OPTRN_LEG_v1.sql  
...
This document details the two critical enhancements implemented in `TRAIN_OPTRN_LEG_v1.sql` based on production readiness review findings.
...
```sql
BEGIN
    SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.TRAIN_OPTRN_LEG_BASE_HIST_STREAM') 
    INTO v_stale_after;
...
WHERE TABLE_NAME = 'TRAIN_OPTRN_LEG' 
...
3. **Verify**: Run `CALL D_RAW.SADB.SP_PROCESS_TRAIN_OPTRN_LEG()` and check log entry

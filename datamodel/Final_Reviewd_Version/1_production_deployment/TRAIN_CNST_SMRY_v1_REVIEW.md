# TRAIN_CNST_SMRY v1 Enhancement Review

**Document Date:** March 13, 2026  
**Original Version:** TRAIN_CNST_SMRY.sql  
**Enhanced Version:** TRAIN_CNST_SMRY_v1.sql  
...
This document details the two critical enhancements implemented in `TRAIN_CNST_SMRY_v1.sql` based on production readiness review findings.

**Primary Key Type:** COMPOSITE (TRAIN_CNST_SMRY_ID, TRAIN_CNST_SMRY_VRSN_NBR) - 2 columns
...
```sql
BEGIN
    SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.TRAIN_CNST_SMRY_BASE_HIST_STREAM') 
    INTO v_stale_after;
...
INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
    ...
    'TRAIN_CNST_SMRY', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time,
...
This table uses a **2-column composite primary key**: `(TRAIN_CNST_SMRY_ID, TRAIN_CNST_SMRY_VRSN_NBR)`
...
WHERE TABLE_NAME = 'TRAIN_CNST_SMRY' 
...
3. **Verify**: Run `CALL D_RAW.SADB.SP_PROCESS_TRAIN_CNST_SMRY()` and check log entry

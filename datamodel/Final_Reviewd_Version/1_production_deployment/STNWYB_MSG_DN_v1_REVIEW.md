# STNWYB_MSG_DN v1 Enhancement Review

**Document Date:** March 13, 2026  
**Original Version:** STNWYB_MSG_DN.sql  
**Enhanced Version:** STNWYB_MSG_DN_v1.sql  
...
This document details the two critical enhancements implemented in `STNWYB_MSG_DN_v1.sql` based on production readiness review findings.
...
```sql
BEGIN
    SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.STNWYB_MSG_DN_BASE_HIST_STREAM') 
    INTO v_stale_after;
...
WHERE TABLE_NAME = 'STNWYB_MSG_DN' 
...
3. **Verify**: Run `CALL D_RAW.SADB.SP_PROCESS_STNWYB_MSG_DN()` and check log entry

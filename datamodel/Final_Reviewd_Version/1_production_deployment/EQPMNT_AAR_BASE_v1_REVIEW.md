# EQPMNT_AAR_BASE v1 Enhancement Review

**Document Date:** March 13, 2026  
**Original Version:** EQPMNT_AAR_BASE.sql  
**Enhanced Version:** EQPMNT_AAR_BASE_v1.sql  
...
This document details the two critical enhancements implemented in `EQPMNT_AAR_BASE_v1.sql` based on production readiness review findings.

**Important Note:** This table has a special naming convention where the source table is `EQPMNT_AAR_BASE_BASE` (double _BASE suffix) while the target table is `EQPMNT_AAR_BASE`. The stream name follows the source: `EQPMNT_AAR_BASE_BASE_HIST_STREAM`.
...
```sql
BEGIN
    SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.EQPMNT_AAR_BASE_BASE_HIST_STREAM') 
    INTO v_stale_after;
...
INSERT INTO D_BRONZE.MONITORING.CDC_EXECUTION_LOG (
    ...
    'EQPMNT_AAR_BASE', :v_batch_id, 'SUCCESS', :v_start_time, :v_end_time,
...
|-------------|------|-------|
| Source Table | D_RAW.SADB.EQPMNT_AAR_BASE_BASE | Double _BASE suffix |
| Target Table | D_BRONZE.SADB.EQPMNT_AAR_BASE | Standard naming |
| Stream | D_RAW.SADB.EQPMNT_AAR_BASE_BASE_HIST_STREAM | Follows source naming |
| Log TABLE_NAME | 'EQPMNT_AAR_BASE' | Uses target table name |
...
WHERE TABLE_NAME = 'EQPMNT_AAR_BASE' 
...
3. **Verify**: Run `CALL D_RAW.SADB.SP_PROCESS_EQPMNT_AAR_BASE()` and check log entry

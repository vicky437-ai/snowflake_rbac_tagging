# TRAIN_CNST_DTL_RAIL_EQPT v1 Enhancement Review

**Document Date:** March 13, 2026  
**Original Version:** TRAIN_CNST_DTL_RAIL_EQPT.sql  
**Enhanced Version:** TRAIN_CNST_DTL_RAIL_EQPT_v1.sql  
...
This document details the two critical enhancements implemented in `TRAIN_CNST_DTL_RAIL_EQPT_v1.sql` based on production readiness review findings.

**Primary Key Type:** COMPOSITE (TRAIN_CNST_SMRY_ID, TRAIN_CNST_SMRY_VRSN_NBR, SQNC_NBR) - **3 columns**
...
```sql
BEGIN
    SELECT SYSTEM$STREAM_GET_STALE_AFTER('D_RAW.SADB.TRAIN_CNST_DTL_RAIL_EQPT_BASE_HIST_STREAM') 
    INTO v_stale_after;
...
This table uses a **3-column composite primary key**: `(TRAIN_CNST_SMRY_ID, TRAIN_CNST_SMRY_VRSN_NBR, SQNC_NBR)`

The MERGE ON clause correctly joins on **all three columns**:
```sql
...
**IMPORTANT**: This is the most complex PK pattern among the 4 composite PK tables, requiring 3 join conditions in every MERGE operation.
...
3. **Verify**: Run `CALL D_RAW.SADB.SP_PROCESS_TRAIN_CNST_DTL_RAIL_EQPT()` and check log entry

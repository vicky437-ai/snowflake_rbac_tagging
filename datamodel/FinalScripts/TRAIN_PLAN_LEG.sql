...
Task         : D_RAW.SADB.TASK_PROCESS_TRAIN_PLAN_LEG
...
-- =============================================================================
CREATE TABLE IF NOT EXISTS D_BRONZE.SADB.TRAIN_PLAN_LEG (
...
BEGIN
    ...
    IF (v_stream_stale = TRUE) THEN
        ...
        USING (
            ...
            FROM D_RAW.SADB.TRAIN_PLAN_LEG_BASE src
            LEFT JOIN D_BRONZE.SADB.TRAIN_PLAN_LEG tgt 
                ON src.TRAIN_PLAN_LEG_ID = tgt.TRAIN_PLAN_LEG_ID
            WHERE tgt.TRAIN_PLAN_LEG_ID IS NULL
        ...
        WHEN NOT MATCHED THEN INSERT (
            ...
            MTP_ROUTE_CMPLT_CD, MTP_TRAIN_STATE_CD, SNW_OPERATION_TYPE, SNW_LAST_REPLICATED,
            ...
            src.MTP_ROUTE_CMPLT_CD, src.MTP_TRAIN_STATE_CD, src.SNW_OPERATION_TYPE, src.SNW_LAST_REPLICATED,
...
-- =============================================================================
CREATE OR REPLACE TASK D_RAW.SADB.TASK_PROCESS_TRAIN_PLAN_LEG
    WAREHOUSE = INFA_INGEST_WH
    SCHEDULE = '5 MINUTE'
    ALLOW_OVERLAPPING_EXECUTION = FALSE
    COMMENT = 'Task to process TRAIN_PLAN_LEG_BASE CDC changes into data preservation table'
WHEN
    SYSTEM$STREAM_HAS_DATA('D_RAW.SADB.TRAIN_PLAN_LEG_BASE_HIST_STREAM')
AS
...
-- SHOW TASKS LIKE 'TASK_PROCESS_TRAIN_PLAN_LEG%' IN SCHEMA D_RAW.SADB;
-- CALL D_RAW.SADB.SP_PROCESS_TRAIN_PLAN_LEG();
